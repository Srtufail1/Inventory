...
import { updateOutwardNotes } from "@/actions/outward-notes";
...
const ExpandableNoteRow: React.FC<ExpandableNoteRowProps> = ({
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const result = await updateOutwardNotes(outwardId, noteText);
      if (result?.error) {
        toast({ title: result.error, variant: "destructive" });
      } else {
        toast({ title: "Notes saved successfully" });
        setHasChanges(false);
        onNotesSaved?.(outwardId, noteText.trim() || null);
        onCollapse?.();
      }
    } catch (error) {
    ...
    <td colSpan={colSpan} className="p-0">
      <div className="bg-amber-50/50 dark:bg-amber-950/20 border-t border-b border-amber-200 dark:border-amber-800/50 px-6 py-4">
        ...
        <textarea
          ...
          placeholder="Add notes for this outward record... (Ctrl+Enter to save)"
          rows={3}
